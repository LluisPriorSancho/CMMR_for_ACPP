...
def generate_launch_description():
    ...
    cont = os.path.join(leo_description_share, 'urdf', 'leo_sim2.urdf.xacro')
    robot_desc = xacro.process_file(cont)#, mappings={"robot":'false'}
    xml = robot_desc.toxml()
    content = {'robot_description': xml}

    # Node to publish the frames of the rover
    ...
    joint_state_broadcaster_controller = Node(
        name="joint_state_broadcaster_spawner_leo2",
    ...
    differential_drive_controller = Node(
        name="diff_drive_controller_spawner_leo2",
        package="controller_manager",
        executable="spawner",
        namespace=LaunchConfiguration("robot_ns"),
        arguments=["leo2_velocity_controller"], #, '-n', LaunchConfiguration("robot_ns")
    )
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                name="model",
                default_value=[leo_description_share, "/urdf/leo_sim2.urdf.xacro"],
